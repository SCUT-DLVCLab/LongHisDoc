...
import re
sys.path.append('/data1/szy/LLM_code/GujiDoc_collections/test_code/infer_code')
# from qwen2_5vl_colpali import Qwen2_5_VLConfig, Qwen2_5_VLForConditionalGeneration
from colpali_engine_.models import ColQwen2_5, ColQwen2_5Processor
...
parser.add_argument('--model_path', type=str, default="colqwen2_5", help='Path to the model')

parser.add_argument('--output_path', type=str, default="./retrivers_res/colqwen2_5.json", help='Path to output json')
...
# model_name = "/home/szy/local/DL_Work_space/LLM_code/models/colqwen2-v1.0-merged"
model_name = "/data1/szy/LLM_code/colpali/save_models/colqwen2_5-merged-v0.2-5e-4-warmup30_100k_guji_2_5/checkpoint-1000"
...
for root, dirs, files in os.walk(image_directory):
    for folder_name in tqdm(dirs, desc="Processing folders"):
        ...
        if os.path.isdir(folder_path) and folder_name in classified_data:
            ...
            all_images_processed = processor.process_images(images) #这个processor函数就是会把数据放到cpu上处理，所以一次处理太多图片还是会爆内存
            # import pdb
            # pdb.set_trace()
...
print(f"recall_score_average_sp:{round(recall_score_average_sp,2)}")
print(f"map_score_sp:{round(map_score_sp,2)}")
print(f"recall_score_average_mp:{round(recall_score_average_mp,2)}")
print(f"map_score_mp:{round(map_score_mp,2)}")
